...
public class MorseCodeDecoder {
	...
	public static String decodeMorse(String inputMorseCode) {
		return Arrays.stream(inputMorseCode.trim().split("   ")).map(e -> MorseCodeDecoder.findEquivalent(e))
				.collect(Collectors.joining(" "));
	}

	public static String findEquivalent(String words) {
		MorseCodeDecoder.put();
		return Arrays.stream(words.split(" ")).map(e -> MorseCodeDecoder.get(e)).collect(Collectors.joining());
	}
