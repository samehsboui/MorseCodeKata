...
public class Decoder {
	...
	public static String decodeBits(final String bits) {
		...
		final String normalizedBits = normalize(bits);
		final String[] bitWords = normalizedBits.split(BIT_WORD_PAUSE);
		for (final String bitWord : bitWords) {
			if (morseBuilder.length() > 0) {
				morseBuilder.append(MORSE_WORD_DELIMITER);
			}
			final StringBuilder wordBuilder = new StringBuilder();
			final String[] bitLetters = bitWord.split(BIT_LETTER_PAUSE);
			for (final String bitLetter : bitLetters) {
				if (wordBuilder.length() > 0) {
					wordBuilder.append(MORSE_LETTER_DELIMITER);
				}
				final StringBuilder letterBuilder = new StringBuilder();
				final String[] bitLetterParts = bitLetter.split(BIT_LETTER_PART_PAUSE);
				for (final String bitLetterPart : bitLetterParts) {
					final char morseSignal = BIT_TO_MORSE.getOrDefault(bitLetterPart, '?');
					letterBuilder.append(morseSignal);
				}
	...
	public static String decodeMorse(final String morseCode) {
		...
		for (final String morseWord : morseCode.trim().split(MORSE_WORD_DELIMITER)) {
			...
			for (final String morseLetter : morseWord.split(MORSE_LETTER_DELIMITER)) {
				decodeBuilder.append(MorseCodeDecoder.get(morseLetter));
			}
		}
		return decodeBuilder.toString();
	}

	static String normalize(final String bits) {
	...
	static int getTransmissionRate(final String bits) {
		if (bits.isEmpty()) {
			return 1;
		}
		...
		if (bitOnCounts.contains(BIT_LETTER_PART_DASH_LENGTH)) {
			return bitOffCounts.contains(BIT_WORD_PAUSE_LENGTH) ? BIT_LETTER_PART_DOT_LENGTH
		...
		candidate: for (final int candidate : transmissionRateCandidates) {
	...
	static String trim(final String s, char c) {
	...
	static String repeat(final char c, final int times) {
